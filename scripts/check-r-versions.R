...
# Reduce a version label to its leading numeric tuple ("X.Y" or "X.Y.Z").
# Strips ALL trailing junk regardless of separator: "3.13.0dev",
# "3.13.0beta2", "3.13.0-rc1", "3.13.0alpha", "3.13.0+gabc123" all -> "3.13.0".
# This is the only part that actually identifies which library was loaded;
# the suffix is a label, not an identity.
normalize <- function(v) {
  v <- strip_codename(v)
  m <- regmatches(v, regexpr("^[0-9]+\\.[0-9]+(\\.[0-9]+)?", v))
  if (length(m) == 0) trimws(v) else m
}
...
check_lib <- function(name, sys_ver, pkg_vers, warn_only = FALSE) {
  ...
  if (ok) {
    ...
    # Note suffix-only differences so canary builds visibly explain themselves
    raw_diffs <- pkg_vers[pkg_vers != sys_ver]
    if (length(raw_diffs) > 0) {
      cat("  note: suffix-only differences (same library, different label):\n")
      cat(sprintf("    system: %s\n", sys_ver))
      for (i in seq_along(raw_diffs)) {
        cat(sprintf("    %-12s %s\n", names(raw_diffs)[i], raw_diffs[i]))
      }
    }
  } else if (warn_only) {
